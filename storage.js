...
// --- Exportar un partido específico (ahora con el MISMO formato que exportarEventos) ---
function exportarPartido(partidoId) {
    ...
    // MISMO encabezado que exportarEventos
    let filas = ["Partido ID,DNI,Jugador,Acción,Tiempo"]
    ...
    eventosPartido.forEach(e => {
        ...
        const partidoIdValue = e.partido || partidoId || ""
        filas.push(`"${partidoIdValue}",${e.dni || ""},"${nombreJugador}","${e.accion}","${e.tiempo}"`)
    })
